...
[extra]
  ...
  toc = true
  top = false
  utrp_data = "/utrp/core/whimsalot.json"
+++
...
> **维护者注：** 翅膀精灵 `spr_whimsalot_wing_r`（17×14）在 GameMaker 中的 origin 为 (0, 14)，即左下角（翅膀与身体的连接处）。翅膀的旋转（±30°）围绕此连接点进行，而非围绕翅膀中心。UTRP 数据中 `pivot` 设为 `(-1, 14)`（x 方向偏移 -1 是因为提取的 PNG 裁剪了 1 列左侧透明像素）。
